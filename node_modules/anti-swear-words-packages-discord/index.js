...
module.exports = (client, message, options) => { 
 ...
 const warnCount = data.get(`warnings_${message.guild.id}_${user.id}`)
 const mutedRole = message.guild.roles.cache.find(role => role.name === muteRole);
 
 if(warnCount === null) {
     //console.log(user.id)
     let warnCount = data.set(`warnings_${message.guild.id}_${user.id}`, 0)
}
...
if (message.content.startsWith(`antiswear!reset`)) { 
    if(!message.member.permissions.has("ADMINISTRATOR")) return message.channel.send(`You dont have the permission to run this command!`);
...
if (foundInTextWord){
    ...
    //console.log(user.id)
      let warnCount = data.add(`warnings_${message.guild.id}_${user.id}`, 1)
          message.delete();
              message.channel.send(`${warnMSG} | You got **${warnCount}** warning(s)!`)
       if(warnCount === muteCount) {
        message.member.timeout(15*60*1000, 'For using swear words too much!');
