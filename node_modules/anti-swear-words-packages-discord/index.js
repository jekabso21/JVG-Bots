   ...
   if(message.member.hasPermission("ADMINISTRATOR")) return;
      let warnCount = data.add(`warnings_${message.guild.id}_${user}`, 1)
       ...
       if(warnCount === muteCount) {
        ...
        //Send him personal message
		const mute = new discord.MessageEmbed()
		.setTitle('User Mute')
		.setColor('#8b6363')
		.addField('Username', `**${member}**`)
		.addField('Muted by', `Bot`)
		.addField('Reason', `For swearing ${muteCount} times!`)
        .setTimestamp()
        message.author.send(mute)
       }
       if(warnCount === kickCount) {
        ...
        .setColor('#000294')
        .addField('Username', `**${member}**`)
        .addField('Kicked by', `BOT`)
        .addField('Reason', `${kickCount} warnings.`)
       ...
       if(warnCount === banCount) {
        ...
        //Ban user for x warnings and swearing
        let reason = "For swearing";    //Reason for the ban
        let userE = new discord.MessageEmbed()
        .setTitle(`You've Been Banned From **${message.guild.name}**`)
        .setColor('#883311')
        .addField('By', `Server Bot`)
        .addField('Reason', `For Useing Swear Words!`)
        .setTimestamp(new Date())
        message.guild.member(member).ban({reason: reason})
        message.author.send(userE);
        db.delete(`warns_${message.guild.id}_${member}`);
        user.ban(reason);
    }
    }
