...
client.on("guildCreate", (guild) => {
  ...
  const joinembed = new Discord.MessageEmbed()
    .setAuthor("Čau es esmu mazais palīgs", client.user.displayAvatarURL())
    .setDescription(
      "**Thank you for inviting me to your server :grin: :heart:** \n**Mention me for help or type ``!help``** \n**Join our server for news of bot updates by typing ``!server``** \n**Your server prefix: ``!``** \n**You can change the bot prefix by typing ``!prefix``**"
    )
    .setThumbnail(
      client.user.displayAvatarURL({ format: "png", dynamic: true, size: 1024 })
    )
